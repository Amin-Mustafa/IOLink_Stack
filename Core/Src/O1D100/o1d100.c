...
void O1D100_Init(O1D100_t *sensor) {
    ...
    sensor->sensor_drv.sensor_ctx = sensor;
    sensor->sensor_drv.pd_handler = O1D100_PD_Handler;
    
    sensor->sensor_drv.isdu_mutex = osMutexNew(NULL);
    sensor->sensor_drv.isdu_sem = osSemaphoreNew(1, 0, NULL);
}
...
bool O1D100_GetVendorName(O1D100_t *sensor, char *out_name, uint16_t max_len) {
    ...
    if (IOLink_Master_ReadISDU(sensor->sensor_drv.port_num, 16, 0, (uint8_t*)out_name, &len, 1000)) {
        out_name[len] = '\0'; 
        return true;
    }
    return false;
...
uint32_t O1D100_GetOperatingHours(O1D100_t *sensor) {
    ...
    if (IOLink_Master_ReadISDU(sensor->sensor_drv.port_num, 542, 0, ((uint8_t*)&buffer), &len, 1000)) {
